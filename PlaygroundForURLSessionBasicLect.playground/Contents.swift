...
let dataTask = session.dataTask(with: requestURL) { (data, response, error) in
    ...
    // - Track 오브젝트를 만들어야 겠다
    // - Dat에서 struct로 파싱하고 싶다 > Codable 이용해서 만들자
    // -  Json file, data > object (Codable 이용하겠다)
    // - Response, [Track] 이렇게 두 개 만들어야 겠다
    
    // 해야 할 일
    // - Response, Track struct
    // - struct의 프로퍼티 이름과 실제 데이터의 키와 맞추기 (Codable 디코딩 하게 하기 위해서)
    // - 파싱하기 (Decoding)
    // - 트랙리스트 가져오기
